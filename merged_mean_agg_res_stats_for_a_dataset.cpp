...
#include <string>
#include <vector>
//this code is acopy of the merged global stats for a dataset and does the same thing but for mean agg stats, it creates
//csv files from 1 to 8 of merged mean agg stats of (mathotp et hexaly)
...
void mergeGlobalStatsCsv(const std::string &hexalyFile,
  ...
  std::string base = "perfs_csv/merged_stats/mean_agg_res_stats/";
  ...
  std::cout << "Merged CSV written to: "
            << "perfs_csv/merged_stats/mean_agg_res_stats/" + outputFile << "\n";
}

int main(int argc, char *argv[]) {
  if (argc < 3) {
    std::cerr << "Usage: " << argv[0]
              << " <stat_mean_agg_h_csv1> <stat_mean_agg_m_csv2> [output_csv]"
              << std::endl;
    return 1;
  }
  ...
  std::string outputFile = argc >= 4 ? argv[3] : "merged_mean_agg_res_stats1.csv";
